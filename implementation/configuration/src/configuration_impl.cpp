            ...
            if (its_key == "console") {
                ...
                for (auto j : i->second) {
                    ...
                    if (its_sub_key == "enable") {
                        ...
                        // for compatibility reasons, this value has to be set
                        // on the config in seconds, and here is converted to milliseconds
                        // for more precision, set it on application level
                        log_version_interval_ *= 1000;
                    }
                }
...
void configuration_impl::load_application_data(const boost::property_tree::ptree& _tree, const std::string& _file_name) {
    ...
    for (auto i = _tree.begin(); i != _tree.end(); ++i) {
        ...
        if (its_key == "name") {
            ...
            its_converter << std::dec << its_value;
            its_converter >> its_log_status_interval;
    ...
    if (its_name != "") {
        if (!(applications_.find(its_name) != applications_.end())) {
            ...
            applications_[its_name] = {its_id,
                                       ...
                                       its_io_thread_count,
                                       its_log_status_interval,
                                       its_log_version_interval,
...
void configuration_impl::load_service_debounce(const boost::property_tree::ptree& _tree, debounce_configuration_t& _debounces) {
    ...
    if (its_service > 0 && its_instance > 0 && !its_debounces.empty()) {
        ...
        if (search != debounces_.end()) {
            VSOMEIP_ERROR << "Multiple debounce configurations for service " << hex4(its_service) << "." << hex4(its_instance);
            return;
        }
