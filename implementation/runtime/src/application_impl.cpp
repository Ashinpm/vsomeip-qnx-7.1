...
bool application_impl::has_active_dispatcher() const {
    ...
    return std::any_of(dispatchers_.begin(), dispatchers_.end(), [this](const auto& d) {
        return !(running_dispatchers_.find(d.first) != running_dispatchers_.end()) && !(elapsed_dispatchers_.find(d.first) != elapsed_dispatchers_.end());
    });
}
...
bool application_impl::is_active_dispatcher(const std::thread::id& _id) const {
    ...
    if (!(dispatchers_.find(_id) != dispatchers_.end())) {
        return false;
    }
    return std::none_of(dispatchers_.begin(), dispatchers_.end(), [&](const auto& d) {
        return d.first != _id && !(running_dispatchers_.find(d.first) != running_dispatchers_.end()) && !(elapsed_dispatchers_.find(d.first) != elapsed_dispatchers_.end());
    });
}
...
void application_impl::send_back_cached_event(service_t _service, instance_t _instance, event_t _event) {
    ...
    if (its_event && its_event->is_field() && its_event->is_set()) {
        ...
        VSOMEIP_INFO << "Sending back cached event (" << hex4(client_) << "): [" << hex4(_service) << "." << hex4(_instance) << "."
                     << hex4(_event) << "]";
    }
}
...
void application_impl::send_back_cached_eventgroup(service_t _service, instance_t _instance, eventgroup_t _eventgroup) {
    ...
    for (const auto& its_event : its_events) {
        if (its_event && its_event->is_field() && its_event->is_set()) {
            ...
            on_message(std::move(its_message));
            VSOMEIP_INFO << "Sending back cached event (" << hex4(client_) << "): [" << hex4(_service) << "." << hex4(_instance) << "."
                         << hex4(its_event_id) << "] from eventgroup " << hex4(_eventgroup);
        }
    }
}
